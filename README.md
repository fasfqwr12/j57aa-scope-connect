...
手机端采用固定一屏外壳，顶栏与底部主 Tab 不随内容滚动；长表单通过页内分组切换，预览按需弹出。短屏、键盘弹出和长日志仅在当前内容区滚动，保留全部操作；桌面恢复原布局。移动布局入口版本为 `mobile-screen-05`。

页面检查覆盖320至1440像素的14种宽度，含临界断点；打开 `tests/browser-smoke.html` 可复现。手机高度、分组、预览和桌面还原检查见 `tests/mobile-screen-smoke.html`，可加 `?width=390&height=844`。九主题可读性检查见 `tests/theme-smoke.html`。自动检查不代表审美认可、全站无障碍认证或真机蓝牙验收。
