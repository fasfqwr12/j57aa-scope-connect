...
| 0xF7 | 业务信息查询 | ✅55B（RT分发） | ✅55B | 0xFE(F7帧) |

APP 侧 0x0A 收不到（WA RX 过滤器仅放行 0x01..0x09，WA:202-209）；但 **0xF7 在 APP 由 RT 分发表条目1 响应**（RT:310,173-202，先于 upgrade_service；IT:421-446 构造同样 55B 帧，出口 usart2）——**F7 两模式都回，不能用来判模式**。APP 侧 0x04~0x09 落 default **不回复**（WA:173-176）。
